...
import { CannotFindOfficeException, HttpException } from '../exceptions'
...
function extendArrayWithId(extend: any, data: any) {
...
async function processWorkflowData(data: any) {
  let data_ = cloneDeep(data)
  if (data_.userid) {
    try {
      const user = await UserModel.findOne({ id: data_.userid })
      if (user) {
        data_.message = `Пользователь ${user.credentials} | ${data_.message}`
      } else {
        data_.message = `${getAnonymousAnimal()} | ${data_.message}`
      }
    } catch (e) {
      data_.message = `${getAnonymousAnimal()} | ${data_.message}`
    }
...
@pre<Order>('save', async function() {
  ...
  // TODO: search customer and create if not exits
})
...
export class Order {
      ...
      arr.push(
        extendArrayWithId(
          ...
          await processWorkflowData({
            message: `${workFlowStart}: ${data?.message}`,
  ...
  private static setHelper(target: 'workflow', arr: any, comment: string) {
    if (target === 'workflow') {
      arr.push(
  ...
  public static async setStatus(
    ...
    const order = await this.findOne({ id })
    order.status = status
    this.setHelper('workflow', order.workflow, `Установлен статус ${status}`)
    return await order.save()
  }
  ...
  public static async setPayed(this: ReturnModelType<typeof Order>, id: number | string, payed: boolean) {
    ...
    order.payed = payed
    this.setHelper('workflow', order.workflow, `Статус оплаты изменён с ${oldPayedStatus} на ${payed}`)
    return await order.save()
  }
  ...
  public static async setMaster(this: ReturnModelType<typeof Order>, id: number | string, master: number) {
    ...
    if (order.masterid) {
      ...
      this.setHelper('workflow', order.workflow, `Выставлен ответственный мастер ${newMaster.credentials}`)
    }
  ...
  public static async setManager(this: ReturnModelType<typeof Order>, id: number | string, manager: number) {
    ...
    if (order.managerid) {
      ...
      this.setHelper('workflow', order.workflow, `Выставлен ответственный менеджер ${newManager.credentials}`)
    }
  ...
  public static async setOffice(this: ReturnModelType<typeof Order>, id: number | string, officeCode: string) {
    ...
    this.setHelper('workflow', order.workflow, `Изменен офис на ${office.code}`)
    return await order.save()
  }
}
