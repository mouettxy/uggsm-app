<template lang="pug">
.timeline-item
  .timeline-item__line(:class='item.color')
  template(v-if='item.type === "solo"')
    .timeline-item__icon(:class='item.color')
    ...
    template(v-if='item.username')
      v-tooltip(left)
        template(#activator='{on, attrs}')
          .timeline-item__icon(:class='item.color', v-on='on', v-bind='attrs')
            v-icon(size='1.5rem', color='white') {{ icon }}
...
export default class ATimelineItem extends Vue {
  ...
  get status() {
    return capitalize(last(this.item.message.split(' ')))
  }
