<template lang="pug">
#print-volgograd-check(style='width: 100%')
  table(style='width: 100%')
    tr
      td(style='text-align: center; font-size: 18px; font-weight: bold; ')
        p(style='margin: 0') Квитанция {{ orderId }}
        p(style='margin: 0') 8 (904) 757-57-59
      td(style='text-align: right')
        p(style='margin: 0') ИП Шепелёва Дарья Павловна. ИНН: 340962229534
        p(style='margin: 0') ОГРНИП: 316344300155022 {{ address }}
  ...
  table(style='width: 100%; border-collapse: collapse; margin-top: 16px')
    thead
      tr
        ...
        td(style='text-align: center; border: 1px solid black; border-collapse: collapse;') Наименование работ и запчастей
        td(style='text-align: center; border: 1px solid black; border-collapse: collapse;') Гарантия
        td(style='text-align: center; border: 1px solid black; border-collapse: collapse;') Кол-во
        td(style='text-align: center; border: 1px solid black; border-collapse: collapse;') Цена, руб
        td(style='text-align: center; border: 1px solid black; border-collapse: collapse;') Сум, руб
    tbody(v-if='works')
      tr(
        v-for='work in works',
        :key='work.id'
      )
        td(style='text-align: center; border: 1px solid black; border-collapse: collapse;') {{ work.id }}
        td(style='text-align: center; border: 1px solid black; border-collapse: collapse;') {{ work.message }}
        td(style='text-align: center; border: 1px solid black; border-collapse: collapse;') -
        td(style='text-align: center; border: 1px solid black; border-collapse: collapse;') 1
        td(style='text-align: center; border: 1px solid black; border-collapse: collapse;') {{ work.price }}
        td(style='text-align: center; border: 1px solid black; border-collapse: collapse;') {{ work.price }}
      tr
        td(
        ...
        td(style='text-align: center; border: 1px solid black; border-collapse: collapse;') {{ worksPrice }}
  div(style='font-size: 16px; margin-top: 24px')
    p(style='margin: 0') Всего наименований {{ worksCount }}, на сумму {{ worksPrice }} руб.
    p(style='margin: 0')
      strong Сумма к оплате (прописью):&nbsp;
      span {{ worksPhrase }}
  div(style='font-size: 16px; margin-top: 44px; text-align: right')
    span Подпись продавца:
    span(style='display: inline-block; border-bottom: 2px solid; width: 160px; margin-left: 4px')
  m-print-adversitement(
    v-bind='$props',
    bonus-type='WhatsApp',
    bonus-text='WhatsApp +7 (904) 757-57-59',
    bonus-target='taggsm'
  )
</template>
